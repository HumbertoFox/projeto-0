...
if (isset($dados['cadastrar_doacao']) || isset($dados['cadastrar_agendar'])) {
    if (!empty($dados['objeto1']) & !empty($dados['quant1'])) {
        ...
        echo "<span class='sucess_php'>Successfully registered donation!</span><br>";

        if (isset($dados['cadastrar_doacao'])) {
            header('Location: ./agen_coleta.php');
        } else {
            header('Location: ./cadastro-doador.php');
        }
    } else {
        echo "<span class='error_php'>Fill in at least one object field and/or one quantity field!</span><br>";
    }
}
